...
class StableDiffusionPipeline:
    ...
    def produce_img_latents_sdxl(
        ...
        for i, t in tqdm(enumerate(total_timesteps)):
            ...
            # }
            # torch.save(input_to_unet, f"shark_input_to_unet49.pt")

            # loaded_data = torch.load(f"input_to_unet49.pt", map_location=torch.device('cpu'))
            # latent_model_input = loaded_data["latent_model_input"].to(torch.float16)
            # prompt_embeds = loaded_data["prompt_embeds"].to(torch.float16)
            # add_text_embeds = loaded_data["add_text_embeds"].to(torch.float16)
            # add_time_ids = loaded_data["add_time_ids"].to(torch.float16)
            ...
            # print(torch.from_numpy(noise_pred))
            # noise_pred = torch.load(f"golden_unet_output49.pt", map_location=torch.device('cpu'))
            # print("======\n")
            # print(noise_pred)
            latents = self.scheduler.step(
    ...
    def decode_latents_sdxl(self, latents):
        ...
        #     "latents": latents
        # }
        # torch.save(input_to_vae, f"shark_input_to_vae.pt")
        # loaded_data = torch.load(f"vae_input.pt", map_location=torch.device('cpu')).to(torch.float32)
        images = self.vae("forward", (latents,))
        # print(torch.from_numpy(images))
        # images = torch.load("golden_vae_output.pt", map_location=torch.device('cpu'))
        # print("=====\n")
        # print(images)
