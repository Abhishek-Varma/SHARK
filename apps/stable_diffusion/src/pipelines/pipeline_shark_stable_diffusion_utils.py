...
class StableDiffusionPipeline:
    def __init__(
        ...
        self.controlnet = UNet2DConditionModel.from_pretrained(
            "/home/abhishek/weights/canny_weight",
        ...
        for i, t in tqdm(enumerate(total_timesteps)):
            ...
            if controlnet_hint is not None:
                if not torch.is_tensor(latent_model_input):
                    latent_model_input_1 = torch.from_numpy(np.asarray(latent_model_input)).to(dtype)
                else:
                    latent_model_input_1 = latent_model_input
                control = self.controlnet(
                    latent_model_input_1, timestep, encoder_hidden_states=text_embeddings, controlnet_hint=controlnet_hint
                )
    ...
    def controlnet_hint_conversion(self, controlnet_hint, height, width, num_images_per_prompt=1):
        ...
        if isinstance(controlnet_hint, torch.Tensor):
            ...
            if controlnet_hint.shape in [shape_chw, shape_bchw, shape_nchw]:
                controlnet_hint = controlnet_hint.to(dtype=torch.float32, device=torch.device("cpu"))
